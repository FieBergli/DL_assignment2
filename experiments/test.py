...
class ReLu(Op):
    """
    ReLU non-linearity: element-wise max(0, x)
    """
    @staticmethod
    def forward(context, input):
        context["relux"] = input > 0
...
class MLP(vg.Module):
    ...
    def __init__(self, input_size, output_size, hidden_mult=4):
        ...
        for layer in [self.layer1, self.layer2]:
            fan_in = layer.w.value.shape[1]  # number of input features
            layer.w.value = np.random.randn(*layer.w.value.shape) * np.sqrt(2.0 / fan_in)
    ...
    def forward(self, input):
       ...
       # first layer
        ...
        #residual = hidden1
        hidden1 = ReLu.do_forward(hidden1)

        # second layer
        hidden2 = self.layer2(hidden1)
        hidden2 = ReLu.do_forward(hidden2)
